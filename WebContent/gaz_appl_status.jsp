...
<html lang="en">
    <head>
        ...
        <title>ICARD: Gaz Applications Status</title>
  <!-- Bootstrap Core CSS -->
...
%>
       ...
       <%
int i=1;
String query="select ID_NO,RUID_NO,EMPNAME,DESIGNATION,DOB,DEPARTMENT,STATION,RES_ADDRESS,RLY_NUMBER,MOBILE_NUMBER,EMERGENCY_CONTACT_NAME,EMERGENCY_CONTACT_NO,PHOTO,SIGNATURE,LAST_UPDATE,dept_name,qr_code,HINDI_NAME,HINDI_DESIG,status,remarks from gaz_master ngm,department d where d.dept_code=ngm.department  and id_no='"+appid+"' and dob='"+dob+"' order by id_no";
 rs=st.executeQuery(query);
 if(rs.next()){
 %> 
                        <div class="panel panel-primary" id="dataTable">
                            <div  class="btn btn-lg btn-primary btn-block">
                                ID No:<%=rs.getString("ID_NO") %> &nbsp;&nbsp;&nbsp;&nbsp;Status= <%=rs.getInt("STATUS")==1?"Pending":rs.getInt("STATUS")==2?"Printing":rs.getInt("STATUS")==3?"Printing (To be Sent)":rs.getInt("STATUS")==4?"Printing (Sent)":rs.getInt("STATUS")==5?"Closed":"Rejected" %> <%=rs.getInt("STATUS")==6?"("+rs.getString("remarks")+")":"" %>
