...
<html lang="en">
    <head>
        ...
        <title>ICARD:Applications (GAZ)</title>
  <!-- Bootstrap Core CSS -->
...
if(from_date.equals("") || to_date.equals("") || from_date==null || to_date==null){
 query="select ID_NO,RUID_NO,EMPNAME,DESIGNATION,DOB,DEPARTMENT,STATION,BILLUNIT,RES_ADDRESS,RLY_NUMBER,MOBILE_NUMBER,EMERGENCY_CONTACT_NAME,EMERGENCY_CONTACT_NO,PHOTO,HINDI_NAME,HINDI_DESIG,SIGNATURE,LAST_UPDATE,dept_name,qr_code,status,remarks from gaz_master gm,department d where d.dept_code=gm.department and status='"+status+"' order by last_update";
}
else{
query="select ID_NO,RUID_NO,EMPNAME,DESIGNATION,DOB,DEPARTMENT,STATION,BILLUNIT,RES_ADDRESS,RLY_NUMBER,MOBILE_NUMBER,EMERGENCY_CONTACT_NAME,EMERGENCY_CONTACT_NO,PHOTO,HINDI_NAME,HINDI_DESIG,SIGNATURE,LAST_UPDATE,dept_name,qr_code,status,remarks from gaz_master gm,department d where d.dept_code=gm.department and last_update between '"+from_date+"' and (TO_DATE('"+to_date+"', 'dd-mon-yyyy')+1) and status='"+status+"' order by last_update";
 ...
 %>  		
										 <tr class="info">
    										...
    										<td><img src="/ICARD_Attachments/<%=rs.getString("HINDI_NAME") %>" style="width:100px;" /></td>
    										<td><img src="/ICARD_Attachments/<%=rs.getString("HINDI_DESIG") %>" style="width:100px;" /></td>
 <% int fcount=0;
 ...
 <%} %>					                	<td><%=rs.getInt("STATUS")==1?"Pending":rs.getInt("STATUS")==2?"Printing (Draft)":rs.getInt("STATUS")==3?"Printing (To be Sent)":rs.getInt("STATUS")==4?"Printing (Sent)":rs.getInt("STATUS")==5?"Closed":"Rejected" %></td>
                                            ...
                                            <td>
         								    ...
         								    <td>
 												<a href="/ICARD/GeneratePDF?appid=<%=rs.getString("ID_NO")%>&isGAZ=Y">																
...
function updateStatus(id_no, status){
	var url="gaz_update_status_form.jsp?id_no="+id_no+"&status="+status;
